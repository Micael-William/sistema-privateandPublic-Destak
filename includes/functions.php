...
if(!function_exists('strcode2utf')){ 
  function strcode2utf($str,$lo=true) {
	...
	$str = preg_replace_callback('/\&\#([0-9]+)\;/m', function($m2){ return !isset($m2[0]) ? '' : code2utf($m2[0], $lo); }, $str);
	$str = preg_replace_callback('/\&\#x([0-9a-fA-F]+)\;/m', function($m2){ return !isset($m2[0]) ? '' : codeHex2utf($m2[0], $lo); }, $str);
	return $str;
  }
}
